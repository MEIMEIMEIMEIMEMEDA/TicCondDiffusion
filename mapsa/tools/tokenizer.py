...
from mapsa.tools.labels import get_label_convertor
from mapsa.tools.labels import LabelConvertor
...
class ToknizerWrapper:
    ...
    def span_targets(self):
        return ["<s>", "<pad>", "</s>", "<B>", "<I>", "<O>", "<POS>", "<NEG>", "<NEU>"]
    ...
    def span_target_ids(self) -> List[int]:
        ...
        self._span_target_ids = self._tokenizer.convert_tokens_to_ids(self.span_targets)
        assert len(self._span_target_ids) == len(
            self.span_targets
        ), "Please add target by self.add_special_tokens()"
        return self._span_target_ids
    ...
    @property
    def in_bio_mode(self) -> bool:
        return len(self.reverse_labels_mapping) > 4
    ...
    def _parse(self, tok_output: TokenizerOutput, rt_type="np") -> TokenizerOutput:
        tok_output.parse(
            "token_ids",
            self.bos_token_id,
            self.eos_token_id,
            ...
            "token_labels",
            self.ignore_index,  # torch ignore label
            self.ignore_index,
            self.max_length,
            self.ignore_index,
        ).parse(
    ...
    def get_input_word_ids(self, tokenized_inputs: BatchEncoding) -> List[List[int]]:
        return [
            [
                self.ignore_index if wid is None else wid
    ...
    def token_label_to_span(
        ...
        for i, tk_label_id in enumerate(token_label_ids):
            ...
            if self.in_bio_mode:
                ...
                # TODO(huangwenfeng): refactor
                if pre_tk_label_id != tk_label_id or i == 0:
                    res.append([i + 1, i + 1, tk_label_id])
                else:
                    res[-1][1] += 1
            pre_tk_label_id = tk_label_id
    ...
    def word_labels_align_token_labels(
        self, word_ids: List[int], word_labels: List[str]
    ) -> List[List[int]]:
        # ignore default: -100
        token_labels = np.ones(len(word_ids)) * self.ignore_index
        pre_wid = None
    ...
    def gen_labels(
        ...
        words_labels: List[List[str]],
    ) -> Dict[str, np.ndarray]:
        token_labels = []
        seq_token_labels = []
        span_labels = []
        for words, word_ids, word_labels in zip(
            input_words, input_token_word_ids, words_labels
        ):
            token_labels.append(
        ...
        span_labels = [list(chain(*lb)) for lb in span_labels]
        return {
    ...
    def gen_decoder_inputs(
        self, encode_input_ids: List[np.ndarray], labels: List[np.ndarray]
    ):
        decoder_input_ids = []
        attention_mask = []
    ...
    def encode(
        ...
        if not skip_labels:
            ...
            decoder_inputs = self.gen_decoder_inputs(
                inputs["input_ids"], labels["seq_token_labels"]
            )
    ...
    def decode_from_output(self, tok_output: TokenizerOutput):
        return [
            (
                self.decode(word_tok_ids, skip_special_tokens=True),
...
if __name__ == "__main__":
    ...
    from mapsa.tools.labels import register

    tks = [
        AutoTokenizer.from_pretrained(
            "/nfs/volume-382-182/hub/t5-base", add_prefix_space=True
        ),
        # AutoTokenizer.from_pretrained('google/flan-t5-base',
        #                               add_prefix_space=True)
    ]
    labels_mapping, reverse_labels_mapping = register[LabelType.BIO_SENTIMENT].copy()
    # TODO
    for tk in tks:
        t = ToknizerWrapper(tk, labels_mapping, reverse_labels_mapping)
        # _words = [['A', 'humanaaa', 'likebb', 'me', 'todayaaa', 'me'],
        #           ['A', 'humanaaa', 'likebb', 'me', 'todayaaa', 'me', 'sdfsd']]
        # _label = [['O', 'B-POS', 'I-POS', 'O', 'B-NEU', 'B-NEG'],
        #           ['O', 'B-POS', 'I-POS', 'O', 'B-NEU', 'B-NEG', 'O']]
        # _tok_output = t.encode(_words, _label)
        # _mask = _tok_output.attention_mask[0]
        # _mask[2] = 0
        # word_mask = t.token_masks_to_word_masks(_tok_output.word_ids[0], _mask)
        # print('========={}=========='.format(tk.__class__))
        # print(_tok_output)
        # print(t.decode_from_output(_tok_output))
        # print(word_mask)
        # print(_tok_output.attention_mask[0])
        # print(t.word_masks_to_token_masks(_tok_output.word_ids[0], word_mask))
        print(t.words2inputs("in the image"))
        print(t.words2inputs("not"))
        print(t.decode([59]))
